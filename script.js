...
const cache = {
    ...
    get(key) {
        if (!this.tenants.has(key)) return null;
        if (Date.now() - this.lastUpdated.get(key) > this.CACHE_DURATION) {
...
const batchOperations = {
    ...
    async executeBatch() {
        ...
        for (const [key, data] of this.pendingUpdates) {
            const docRef = db.collection('tenants').doc(key);
...
async function loadTenants() {
    try {
        // Check cache first
        const cachedData = cache.get('tenants');
        if (cachedData) {
...
async function addTenant(tenantData) {
    try {
        ...
        // Update cache
        const cachedTenants = cache.get('tenants') || [];
        cache.set('tenants', [...cachedTenants, newTenant]);
        
        displayTenants(cache.get('tenants'));
        showMessage('Tenant added successfully');
    } catch (error) {
        console.error('Error adding tenant:', error);
        showMessage('Error adding tenant', 'error');
    }
}
...
async function updateTenant(tenantId, updates) {
    try {
        ...
        batchOperations.addUpdate(tenantId, updates);
        
        // Update cache
        const cachedTenants = cache.get('tenants') || [];
        const updatedTenants = cachedTenants.map(tenant => 
            tenant.id === tenantId ? { ...tenant, ...updates } : tenant
        );
        cache.set('tenants', updatedTenants);
        
        displayTenants(updatedTenants);
        showMessage('Tenant updated successfully');
    } catch (error) {
        console.error('Error updating tenant:', error);
        showMessage('Error updating tenant', 'error');
    }
}

async function addPayment(tenantId, paymentData) {
    try {
        const paymentRef = await db.collection('tenants').doc(tenantId)
        ...
        const updatedTenants = cachedTenants.map(tenant => {
            if (tenant.id === tenantId) {
                return {
                    ...tenant,
                    payments: [...(tenant.payments || []), { id: paymentRef.id, ...paymentData }]
                };
            }
            return tenant;
        });
...
function setupOfflineSupport() {
    ...
    window.addEventListener('offline', () => {
        showMessage('You are offline - Changes will be synced when online', 'warning');
    });
}
...
async function searchTenants(query) {
    ...
    try {
        ...
        displayTenants(results);
    } catch (error) {
        console.error('Error searching tenants:', error);
        showMessage('Error searching tenants', 'error');
    }
}

// Optimized Payment History Loading
async function loadPaymentHistory(tenantId) {
    const cacheKey = `payments_${tenantId}`;
    const cachedPayments = cache.get(cacheKey);
    if (cachedPayments) {
        displayPaymentHistory(cachedPayments);
        return;
    }
...
async function calculateStatistics() {
    const cachedStats = cache.get('statistics');
    if (cachedStats) {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            stats.totalTenants++;
            stats.totalRevenue += tenant.totalPaid || 0;
        ...
        const totalRooms = 20; // Assuming 20 rooms total
        stats.occupancyRate = (stats.totalTenants / totalRooms) * 100;

        cache.set('statistics', stats);
        displayStatistics(stats);
    } catch (error) {
        console.error('Error calculating statistics:', error);
        showMessage('Error calculating statistics', 'error');
    }
}

// Initialize search index when tenants are loaded
function initializeSearchIndex(tenants) {
    searchIndex.buildIndex(tenants);
}

// Update the loadTenants function to initialize search index
async function loadTenants() {
    try {
        const cachedData = cache.get('tenants');
        if (cachedData) {
            displayTenants(cachedData);
            initializeSearchIndex(cachedData);
            return;
        }
